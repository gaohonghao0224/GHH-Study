package com.example.springwebsocket.spring.utils;
...
public class RedisUtils implements ApplicationContextAware {
    ...
    /**
     *  静态方法属性 在 spring生命周期之前，故 Autowired无法注入，
     *  使用 Aware接口获取容器直接赋值， 由于Aware接口方法的执行是在 Bean实例化之后回调方法中，
     *  所以要将此工具类变成Bean  -- @Component，redisTemplate 才能赋值
     *
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        RedisUtils.redisTemplate =  applicationContext.getBean("stringRedisTemplate",RedisTemplate.class);
    }
    ...
    public static boolean setnx(String key, Object value, long time) {
        try {
            if (time > 0) {
                redisTemplate.opsForValue().setIfAbsent(key, String.valueOf(value), time, TimeUnit.SECONDS);
            } else {
                set(key, value);
            }
            return true;
